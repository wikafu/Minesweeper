export default async function handler(req, res) {
  ...
  // if Upstash is not configured, just always say "no winners yet"
  if (!UPSTASH_REDIS_REST_URL || !UPSTASH_REDIS_REST_TOKEN) {
    if (req.method === 'GET') {
      const date = getTodayUTCDateString();
      return res.status(200).json({ date, best: null, note: 'upstash not configured' });
    }
  ...
  const date = getTodayUTCDateString();
  const key = `minesweeper:daily:${date}`;
